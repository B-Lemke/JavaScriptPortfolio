...
function setup() {
    var can = createCanvas(canvasWidth, canvasHeight);
    can.parent('sketchWrapper');

    background(50);

    //add empty paragraph below canvas for spacing
    var firstp = createP('Left Branch Angle:');
    firstp.parent('sketchWrapper');

    //create slider to adjust branchAngle
    //minVal, maxVal, startVal, stepSize
    leftAngleSlider = createSlider(0, 2 * PI, PI / 8, PI / 32);
    leftAngleSlider.parent('sketchWrapper');

    var secp = createP('Right Branch Angle')
    secp.parent('sketchWrapper');

    rightAngleSlider = createSlider(0, 2 * PI, PI / 8, PI / 32);
    rightAngleSlider.parent('sketchWrapper');

    //create
    windBtn = createButton('Toggle Wind');
    windBtn.parent('sketchWrapper');
